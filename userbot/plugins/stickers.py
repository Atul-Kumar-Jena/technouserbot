...
CUSTOM_STICKER_NAME =str(CUSTOM_STICKER_PACK_NAME) if CUSTOM_STICKER_PACK_NAME else "My Boss Volume Pack"
...
async def _(event):
    ...
    if is_a_s:
        ...
        if userid == 1250027204:
            packname = f"Mr. Pro Ka Pack"
            packshortname = "SKSisgreat"
        else:
            packname = f"{user.username}'s {pack}"
            packshortname = f"Techno_{userid}" # format: Uni_Borg_userid
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    ...
                    #if userid == 948408212:
                       # packname = f"{user.username}'s {pack}"
                       # packshortname = "Vol._{pack}_Techno_ke_locker_me"
                   # else:
                       # packname = f"Vol._{pack}_Techno{userid}"
                        #packshortname = f"Vol._{pack}_Techno_{userid}_ns"
                    if not await stickerset_exists(bot_conv, packshortname):
